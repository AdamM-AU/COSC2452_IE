...
public class BackEnd {
	...
	private int targetRow; // Users row in CSV
	 ...
	 * 
	 *  MODIFIED: when no filepath is given we will use the onStartCSV filepath
	 *  
	 */
	
	public void loadCSV(String filepath) {
		if (filepath.isEmpty()) {
			filepath = this.onStartCSV;
		}
	...
	public int findUser(String name) {
		name = name.toLowerCase(); // Make input lowercase
		
		int currentRow = 0;
		while (currentRow < this.data.length) {
			String dbName = this.data[currentRow].getSeq_chars();

			if (name.equals(dbName.toLowerCase())) {
				// Match Found
				this.targetRow = currentRow; // save int row to class variable
	...
	// This method is no longer needed as we cannot create new records as a user/student
	...
	// ModifyData
	public void modInputs(String[] inputs) {
		//this.growArray(); // Increase storage array size and set this.counter and increment this.arraySize
		// Copy and type convert inputs[]
		Data tmp = this.data[this.targetRow];
		this.data[this.targetRow] = new Data(tmp.getSeq_chars(), 
										   inputs[0].charAt(0),
										   Integer.parseInt(inputs[1]), 
										   Double.parseDouble(inputs[2]), 
										   Boolean.parseBoolean(inputs[3]));
		this.writeCSV(this.onStartCSV);
	}
	
	// Wipe Data - re-purposed code, not used stand-alone
	// used as part of the refresh task, to clear the data array before re-populating it with the CSV contents 
